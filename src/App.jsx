...
function getJuniorBaseRate(n) {
  if (n === 1) return 210; if (n === 2) return 190; if (n === 3) return 170; return 150;
}
function calcFamilyBilling(family, persons, enrollments, semMonths) {
  ...
  let lineItems = [];
  juniors.forEach(e => { const p = persons.find(x => x.id === e.personId); lineItems.push({ enrollmentId: e.id, personId: e.personId, name: p ? `${p.firstName} ${p.lastName}` : "-", program: "juniors", base: juniorRate, discounted: juniorRate, monthlyRate: null }); });
  adults.forEach(e => { const p = persons.find(x => x.id === e.personId); const semBase = (e.monthlyRate || 0) * months; lineItems.push({ enrollmentId: e.id, personId: e.personId, name: p ? `${p.firstName} ${p.lastName}` : "-", program: e.program, base: semBase, discounted: semBase, monthlyRate: e.monthlyRate }); });
...
function AddressInput({ value, onChange }) {
  ...
  function pick(h) {
    ...
    const city = p.city || p.town || p.village || p.hamlet || p.municipality || p.county || "";
    ...
    onChange({ street: streetFinal, city, province, postal });
  }
  return (
    <div style={{ position: "relative" }}>
      <div style={{ position: "relative" }}>
        <input value={raw} onChange={e => { const v = e.target.value; setRaw(v); onChange({ ...(value || {}), street: v }); clearTimeout(timer.current); timer.current = setTimeout(() => search(v), 420); }}
          onBlur={() => setTimeout(() => setHits([]), 180)}
          placeholder="e.g. 123 Main Street, Ottawa"
      ...
      {hits.length > 0 && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1.5px solid #1a6b3a", borderTop: "none", borderRadius: "0 0 10px 10px", zIndex: 400, maxHeight: 260, overflowY: "auto", boxShadow: "0 8px 28px rgba(0,0,0,0.13)" }}>
          {hits.map((h, i) => {
            ...
            let houseNum = p.housenumber || "";
            if (!houseNum) { const m = lastQuery.current.match(/^(\d+[A-Za-z]?)\s/); if (m) houseNum = m[1]; }
            const streetLine = [houseNum, road].filter(Boolean).join(" ") || road || "-";
            const cityLine = [p.city || p.town || p.village, p.state, p.postcode].filter(Boolean).join(", ");
            return (
              <div key={i} onMouseDown={e => { e.preventDefault(); pick(h); }}
                style={{ padding: "9px 14px", fontSize: 13, cursor: "pointer", borderBottom: "1px solid #f0ede6" }}
                onMouseEnter={e => e.currentTarget.style.background = "#f0faf4"}
                onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
                <div style={{ fontWeight: 600 }}>{streetLine}</div>
                <div style={{ color: "#999", fontSize: 12 }}>{cityLine}</div>
              </div>
            );
          })}
        </div>
...
function PhoneInput({ value, onChange, placeholder }) {
  ...
  return (
    <div>
      <input value={fmtPhone(value)} onChange={e => { const c = cleanPhone(e.target.value); onChange(c); setErr(c && c.length < 10 ? "Must be 10 digits" : ""); }}
        placeholder={placeholder || "(613) 000-0000"}
        style={{ width: "100%", padding: "10px 13px", border: `1.5px solid ${err ? "#e74c3c" : "#ddd"}`, borderRadius: 8, fontSize: 15, fontFamily: "inherit", background: "#fafafa", color: "#1a1a1a" }} />
      {err && <div style={{ fontSize: 11, color: "#e74c3c", marginTop: 2 }}>{err}</div>}
    </div>
  );
}
...
function EmailInput({ value, onChange, placeholder }) {
  ...
  return (
    <div>
      <input value={value} type="email" onChange={e => { onChange(e.target.value); setErr(e.target.value && !validEmail(e.target.value) ? "Invalid email" : ""); }}
...
function ReceiptView({ person, enrollment, payment, receiptNum, semesterLabel, onClose }) {
  const bal = enrollBalance(enrollment);
  const isCredit = bal < 0;
  function doEmail() {
    const recipientEmail = person && person.age >= 18 ? (person.email || "") : (person && (person.parent1Email || person.parent2Email || ""));
    const recipientName = person && person.age >= 18 ? `${person.firstName} ${person.lastName}` : (person && person.parent1First ? `${person.parent1First} ${person.parent1Last || ""}` : "Parent");
    const subject = `Tarteel Receipt #${receiptNum} - ${person ? `${person.firstName} ${person.lastName}` : ""}`;
    const body = [`Dear ${recipientName},`, "", "Please find your payment receipt below.", "", SCHOOL_NAME, SCHOOL_ADDRESS, "", `Receipt #${receiptNum}`, `Issued: ${fmtDate(today())}`, "", `Student #: ${(person && person.studentNum) || "-"}`, `Student: ${person ? `${person.firstName} ${person.lastName}` : "-"}`, `Program: ${PROGRAMS[enrollment.program] || ""}`, `Teacher: ${enrollment.teacherName || "-"}`, `Semester: ${semesterLabel}`, `Payment Date: ${fmtDate(payment && payment.date)}`, `Method: ${(payment && payment.method) || "-"}`, `Amount Paid: $${Number((payment && payment.amount) || 0).toFixed(2)}`, (payment && payment.note) ? `Note: ${payment.note}` : "", "", `Semester Total: $${(enrollment.semesterTotal || 0).toFixed(2)}`, `Remaining Balance: $${Math.abs(bal).toFixed(2)}${isCredit ? " (credit)" : ""}`, "", SCHOOL_NAME, SCHOOL_ADDRESS, "tarteel.ca", "", "Jazakum Allahu Khairan"].join("\n");
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, padding: 16 }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: "#fff", borderRadius: 14, width: 600, maxWidth: "98vw", maxHeight: "94vh", overflowY: "auto", padding: "28px 24px", boxShadow: "0 24px 64px rgba(0,0,0,0.25)", fontFamily: "Georgia, serif" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, borderBottom: "3px solid #1a6b3a", paddingBottom: 14, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#1a6b3a" }}>TARTEEL</div>
            <div style={{ fontSize: 11, color: "#888" }}>Ottawa Quran Institute</div>
            <div style={{ fontSize: 11, color: "#aaa" }}>{SCHOOL_ADDRESS}</div>
          </div>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16 }}>
          <tbody>
            {rows.map((row, i) => (
              <tr key={i}><td style={{ padding: "7px 8px", fontSize: 12, color: "#888", width: "44%", borderBottom: "1px solid #f0ede6" }}>{row[0]}</td><td style={{ padding: "7px 8px", fontSize: 13, fontWeight: 600, borderBottom: "1px solid #f0ede6" }}>{row[1]}</td></tr>
            ))}
            ...
            <tr><td style={{ padding: "7px 8px", fontSize: 14, fontWeight: 700 }}>Remaining Balance</td><td style={{ padding: "7px 8px" }}><span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 20, fontSize: 13, fontWeight: 700, background: bal > 0 ? "#fdecea" : "#e8f5ee", color: bal > 0 ? "#c0392b" : "#1a6b3a" }}>{`$${Math.abs(bal).toFixed(2)}${isCredit ? " (credit)" : ""}`}</span></td></tr>
          </tbody>
        </table>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const matches = persons.filter(p => {
      ...
      const fields = [p.phone, p.email, p.parent1Phone, p.parent1Email, p.parent2Phone, p.parent2Email].filter(Boolean).map(s => s.replace(/\D/g, "") || s.toLowerCase());
      return fields.some(fld => queries.some(q => q.length > 4 && fld.includes(q)));
    });
    f("matchSuggestions", matches.slice(0, 3));
  ...
  function handleSearch(q) {
    f("searchQuery", q);
    if (!q.trim()) { f("matchSuggestions", []); return; }
    const ql = q.toLowerCase();
    f("matchSuggestions", persons.filter(p => (p.firstName + " " + p.lastName).toLowerCase().includes(ql) || (p.parent1First + " " + p.parent1Last).toLowerCase().includes(ql) || (p.parent2First + " " + p.parent2Last).toLowerCase().includes(ql)).slice(0, 6));
  }
  function handleLookup(q) {
    setLookupQuery(q);
    if (!q.trim()) { setLookupResults([]); return; }
    const ql = q.toLowerCase().replace(/[\s\-(). ]/g, "");
    setLookupResults(persons.filter(p => { const fields = [p.phone, p.email, p.parent1Phone, p.parent1Email, p.parent2Phone, p.parent2Email, p.firstName + p.lastName, p.parent1First + p.parent1Last, p.parent2First + p.parent2Last].map(x => (x || "").toLowerCase().replace(/[\s\-(). ]/g, "")); return fields.some(fd => fd.includes(ql)); }));
  }
  function openEditEnrollment(enrollment) {
    const person = persons.find(p => p.id === enrollment.personId);
    if (!person) return;
  ...
  function submitEnrollment() {
    ...
    setFamilies(prev => { let u = prev.slice(); if (!familyId) { const existing = u.find(fm => (mcPhone && fm.phone && fm.phone === mcPhone) || (mcEmail && fm.email && fm.email === mcEmail)); if (existing) familyId = existing.id; } if (familyId) { return u.map(fm => fm.id === familyId ? { ...fm, personIds: [...new Set([...(fm.personIds || []), personId])] } : fm); } familyId = uid(); return [...u, { id: familyId, key: mcPhone || mcEmail || personId, name: mcName, phone: mcPhone, email: mcEmail, personIds: [personId] }]; });
    ...
    if (form.editingEnrollId) { setEnrollments(prev => prev.map(e => e.id !== form.editingEnrollId ? e : { ...e, teacherId: form.teacherId, teacherName: form.teacherName, level: form.level, levelName: form.levelName, monthlyRate: form.monthlyRate || 0 })); setForm(INIT_FORM); setView("dashboard"); return; }
  ...
  function addPayment() {
    ...
    setEnrollments(prev => prev.map(e => { if (e.id !== paymentModal.enrollmentId) return e; const rawPaid = customBal !== "" ? (e.semesterTotal - parseFloat(customBal)) : (e.amountPaid || 0) + amt; updatedEnroll = { ...e, amountPaid: Math.round(rawPaid * 100) / 100, paymentHistory: [...(e.paymentHistory || []), entry] }; return updatedEnroll; }));
    if (updatedEnroll) { const person = persons.find(p => p.id === updatedEnroll.personId); setReceiptModal({ person, enrollment: updatedEnroll, payment: entry, receiptNum: nextReceiptNum(person) }); }
    setInstalment({ amount: "", method: "Cash", date: today(), note: "" }); setCustomBal(""); setPaymentModal(null);
  }
  ...
  function saveEditedPayment() {
    ...
    setEnrollments(prev => prev.map(e => { if (e.id !== editPaymentModal.enrollmentId) return e; const newHistory = (e.paymentHistory || []).map(h => h.id !== editPaymentModal.paymentId ? h : { ...h, amount: newAmt, date: editPaymentForm.date, method: editPaymentForm.method, note: editPaymentForm.note }); const newAmountPaid = (e.paymentType === "full" || e.paymentType === "waived") ? e.semesterTotal : Math.round(newHistory.reduce((a, h) => a + (h.amount || 0), 0) * 100) / 100; return { ...e, paymentHistory: newHistory, amountPaid: newAmountPaid }; }));
    setEditPaymentModal(null);
  }
  function deletePayment(enrollmentId, paymentId) { if (!window.confirm("Delete this payment entry?")) return; setEnrollments(prev => prev.map(e => { if (e.id !== enrollmentId) return e; const newHistory = (e.paymentHistory || []).filter(h => h.id !== paymentId); const newAmountPaid = (e.paymentType === "full" || e.paymentType === "waived") ? e.semesterTotal : Math.round(newHistory.reduce((a, h) => a + (h.amount || 0), 0) * 100) / 100; return { ...e, paymentHistory: newHistory, amountPaid: newAmountPaid }; })); }
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .bp{background:var(--g);color:#fff}.bp:hover{background:#145530}
    ...
    .mc-box{border:2px solid #ddd;border-radius:10px;padding:11px 14px;cursor:pointer;transition:all .15s;user-select:none}
    .mc-box.on{border-color:var(--g);background:#f0faf4}
    ...
    .mc{border:1.5px solid var(--gold);background:#fef9f0;border-radius:10px;padding:11px 14px;cursor:pointer;transition:background .15s;margin-bottom:6px}
    .mc:hover{background:#fef3e0}
    ...
    @media(max-width:767px){
      ...
      .tbl th,.tbl td{padding:7px 7px;font-size:12px}
      .card{padding:14px}
    }
  `;
  ...
  const Sidebar = () => (
    <aside style={{ width: 220, background: "#fff", borderRight: "1px solid var(--border)", padding: "18px 10px", display: "flex", flexDirection: "column", gap: 3, position: "sticky", top: 0, height: "100vh", flexShrink: 0, overflowY: "auto" }}>
      ...
      {NAV_ITEMS.map(n => (
        <div key={n.id} className={`nav${view === n.id ? " on" : ""}`} onClick={() => { setView(n.id); if (n.id !== "enroll") setForm(INIT_FORM); }}>
  ...
  const BottomNav = () => (
    <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", borderTop: "1px solid var(--border)", display: "flex", zIndex: 900, boxShadow: "0 -2px 12px rgba(0,0,0,.08)" }}>
      {NAV_ITEMS.map(n => (
        <div key={n.id} onClick={() => { setView(n.id); if (n.id !== "enroll") setForm(INIT_FORM); }}
          style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "8px 4px 10px", cursor: "pointer", color: view === n.id ? "var(--g)" : "#aaa", borderTop: view === n.id ? "2.5px solid var(--g)" : "2.5px solid transparent", transition: "all .15s", minWidth: 0 }}>
          <span style={{ fontSize: 18, lineHeight: 1 }}>{n.icon}</span>
          <span style={{ fontSize: 9.5, fontWeight: view === n.id ? 700 : 500, marginTop: 3, letterSpacing: 0.2 }}>{n.label}</span>
        </div>
      ))}
    </nav>
  );
  ...
  const MobileHeader = () => (
    <header style={{ position: "sticky", top: 0, zIndex: 800, background: "#fff", borderBottom: "1px solid var(--border)", padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 1px 8px rgba(0,0,0,.06)" }}>
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--g)", lineHeight: 1.2 }}>TARTEEL</div>
        <div style={{ fontSize: 9.5, color: "#bbb", letterSpacing: 1, textTransform: "uppercase" }}>Ottawa Quran Institute</div>
      </div>
  ...
  function renderMain() {
            ...
            <table className="tbl" style={{ minWidth: isMobile ? 500 : "auto" }}>
              ...
              <tbody>
                {progEnrollments(activeProg).map(e => {
                  ...
                  const bal = enrollBalance(e);
                  const lastPay = e.paymentHistory && e.paymentHistory[e.paymentHistory.length - 1];
                  return (
                    <tr key={e.id}>
                      <td><span className="sn">{person.studentNum || "-"}</span></td>
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          {person.photo ? <img src={person.photo} style={{ width: 26, height: 26, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--g)", flexShrink: 0 }} /> : null}
                          <div>
                            <div style={{ fontWeight: 600, fontSize: 13 }}>{`${person.firstName} ${person.lastName}`}</div>
                            {person.hasAllergy && <span style={{ fontSize: 9, background: "#fdecea", color: "#e74c3c", padding: "1px 4px", borderRadius: 3 }}>Allergy</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td><span className={e.paymentType === "full" ? "bgg" : e.paymentType === "waived" ? "bgld" : "bgry"} style={{ fontSize: 10 }}>{ptypeLabel(e.paymentType)}</span></td>
                      <td><span className={bal > 0 ? "brr" : "bgg"} style={{ fontSize: 11 }}>{`$${bal.toFixed(2)}`}</span></td>
                      <td>
                        <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                          <button className="btn bg bxs" onClick={() => openEditEnrollment(e)}>Edit</button>
                          {["instalment", "partial", "discounted"].includes(e.paymentType) && bal > 0 && <button className="btn bgold bxs" onClick={() => setPaymentModal({ enrollmentId: e.id })}>+Pay</button>}
                          {lastPay && <button className="btn bo bxs" onClick={() => issueReceiptFor(e, lastPay)}>Rec.</button>}
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <div className="fade" style={{ padding: pad, maxWidth: 700, margin: "0 auto" }}>
        ...
        {!form.editingEnrollId && (
          <div className="card" style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {PROGRAM_KEYS.map(pk => <div key={pk} className={`ptab ${pk}${form.program === pk ? " on" : ""}`} onClick={() => { f("program", pk); f("teacherId", ""); f("level", ""); f("levelName", ""); if (pk === "brothers") f("gender", "Male"); if (pk === "sisters") f("gender", "Female"); }}>{PROGRAMS[pk]}</div>)}
            </div>
          </div>
        )}
        ...
        {!form.editingEnrollId && (
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            {[{ v: "new", label: "New Person" }, { v: "existing", label: "Existing Person" }].map(m => <div key={m.v} className={`ptab juniors${form.mode === m.v ? " on" : ""}`} onClick={() => { f("mode", m.v); f("searchQuery", ""); f("matchSuggestions", []); f("linkedPersonId", null); }}>{m.label}</div>)}
          </div>
        )}

        {form.mode === "existing" && !form.editingEnrollId && (
          <div className="card" style={{ marginBottom: 14 }}>
            <div className="sec">Search Person</div>
            <div className="fg"><label>Name (student or parent)</label><input value={form.searchQuery} onChange={e => handleSearch(e.target.value)} placeholder="Type any name..." autoFocus /></div>
            {form.matchSuggestions && form.matchSuggestions.length > 0 && <div style={{ marginTop: 10 }}>{form.matchSuggestions.map(p => <div key={p.id} className="mc" onClick={() => autoFill(p)}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span className="sn">{p.studentNum}</span><span style={{ fontWeight: 700 }}>{`${p.firstName} ${p.lastName}`}</span><span style={{ fontSize: 12, color: "#aaa" }}>{`Age ${p.age}`}</span></div><div style={{ fontSize: 12, color: "#aaa", marginTop: 3 }}>{fmtPhone(p.phone) || fmtPhone(p.parent1Phone) || p.email || "-"}</div></div>)}</div>}
          </div>
        )}

        {form.mode === "new" && !form.editingEnrollId && form.matchSuggestions && form.matchSuggestions.length > 0 && (
          <div style={{ background: "#fef9f0", border: "1.5px solid var(--gold)", borderRadius: 10, padding: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--gold)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Existing Record Match</div>
            {form.matchSuggestions.map(p => <div key={p.id} className="mc" onClick={() => linkPerson(p)}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span className="sn">{p.studentNum}</span><span style={{ fontWeight: 700 }}>{`${p.firstName} ${p.lastName}`}</span><span style={{ fontSize: 12, color: "#aaa" }}>{`Age ${p.age}`}</span></div></div>)}
          </div>
        )}
          ...
          <div className="r2" style={{ marginBottom: 12 }}>
            ...
            <div className="fg">
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 2 }}>
                {["Male", "Female"].map(g => { const blocked = (form.program === "brothers" && g === "Female") || (form.program === "sisters" && g === "Male"); return <div key={g} className={`ropt${form.gender === g ? " on" : ""}`} style={{ flex: 1, justifyContent: "center", opacity: blocked ? 0.3 : 1, cursor: blocked ? "not-allowed" : "pointer" }} onClick={() => { if (!blocked) f("gender", g); }}>{g}</div>; })}
              </div>
            </div>
          </div>
          <div className="fg" style={{ marginBottom: 10 }}>
            <label>Address (optional)</label>
            <AddressInput value={form.address} onChange={addr => f("address", addr)} />
          </div>
          {form.address && (form.address.city || form.address.province) && (
            <div className="r3" style={{ marginBottom: 12 }}>
              <div className="fg"><label>City</label><input value={form.address.city || ""} onChange={e => f("address", { ...form.address, city: e.target.value })} /></div>
              <div className="fg"><label>Province</label><input value={form.address.province || ""} onChange={e => f("address", { ...form.address, province: e.target.value })} /></div>
              <div className="fg"><label>Postal</label><input value={form.address.postal || ""} onChange={e => f("address", { ...form.address, postal: e.target.value })} /></div>
            </div>
          )}
          {isAdult && (
            <div style={{ background: "#f0f4ff", border: "1px solid #b7c9f5", borderRadius: 10, padding: 14 }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, color: "var(--blue)", marginBottom: 10 }}>PERSONAL CONTACT (18+)</div>
              <div className="r2">
                <div className="fg"><label>Phone *</label><PhoneInput value={form.phone} onChange={v => f("phone", v)} /></div>
                <div className="fg"><label>Email *</label><EmailInput value={form.email} onChange={v => f("email", v)} /></div>
              </div>
            </div>
          )}
          {isMinor && (
            <div style={{ background: "#f0faf4", border: "1px solid #b7e4cc", borderRadius: 10, padding: 14 }}>
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 14, marginBottom: 8, fontWeight: 500 }}>Both parents available?</div>
                <div style={{ display: "flex", gap: 8 }}>{[{ v: "yes", label: "Yes" }, { v: "no", label: "One parent only" }].map(o => <div key={o.v} className={`ropt${form.twoParents === o.v ? " on" : ""}`} onClick={() => f("twoParents", o.v)}>{o.label}</div>)}</div>
              </div>
              {["parent1", form.twoParents === "yes" ? "parent2" : null].filter(Boolean).map(which => {
                const num = which === "parent1" ? "1" : "2";
                return (
                  <div key={which} style={{ background: "#f8f6f1", borderRadius: 10, padding: 12, marginBottom: 10 }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: "#888", textTransform: "uppercase", marginBottom: 8 }}>{`Parent ${num}${which === "parent1" ? " *" : ""}`}</div>
                    <div className="r2" style={{ marginBottom: 10 }}>
                      <div className="fg"><label>First Name</label><input value={form[which + "First"]} onChange={e => f(which + "First", e.target.value)} /></div>
                      <div className="fg"><label>Last Name</label><input value={form[which + "Last"]} onChange={e => f(which + "Last", e.target.value)} /></div>
                    </div>
                    <div className="r2">
                      <div className="fg"><label>Phone</label><PhoneInput value={form[which + "Phone"]} onChange={v => f(which + "Phone", v)} /></div>
                      <div className="fg"><label>Email</label><EmailInput value={form[which + "Email"]} onChange={v => f(which + "Email", v)} /></div>
                    </div>
                  </div>
                );
              })}
              {form.twoParents === "yes" && (
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "var(--g)", textTransform: "uppercase", marginBottom: 8 }}>Main Contact</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {[{ v: "parent1", label: [form.parent1First, form.parent1Last].filter(Boolean).join(" ") || "Parent 1", sub: fmtPhone(form.parent1Phone) || form.parent1Email || "-" }, { v: "parent2", label: [form.parent2First, form.parent2Last].filter(Boolean).join(" ") || "Parent 2", sub: fmtPhone(form.parent2Phone) || form.parent2Email || "-" }].map(o => (
                      <div key={o.v} className={`mc-box${form.mainContact === o.v ? " on" : ""}`} style={{ flex: 1, minWidth: 140 }} onClick={() => f("mainContact", o.v)}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={form.mainContact === o.v} readOnly /><div><div style={{ fontWeight: 600, fontSize: 13 }}>{o.label}</div><div style={{ fontSize: 11, color: "#aaa" }}>{o.sub}</div></div></div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {form.twoParents === "no" && (
                <div style={{ background: "#fff8e8", border: "1.5px solid #f0d080", borderRadius: 10, padding: 12, marginTop: 10 }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, color: "var(--gold)", textTransform: "uppercase", marginBottom: 8 }}>Emergency Contact Required</div>
                  <div className="r2" style={{ marginBottom: 10 }}>
                    <div className="fg"><label>First Name *</label><input value={form.emergencyFirst} onChange={e => f("emergencyFirst", e.target.value)} /></div>
                    <div className="fg"><label>Last Name *</label><input value={form.emergencyLast} onChange={e => f("emergencyLast", e.target.value)} /></div>
                  </div>
                  <div className="r2">
                    <div className="fg"><label>Phone</label><PhoneInput value={form.emergencyPhone} onChange={v => f("emergencyPhone", v)} /></div>
                    <div className="fg"><label>Relationship</label><select value={form.emergencyRelationship} onChange={e => f("emergencyRelationship", e.target.value)}><option value="">Select...</option>{RELATIONSHIPS.map(r => <option key={r}>{r}</option>)}</select></div>
                  </div>
                </div>
              )}
            </div>
        ...
        {/* Medical */}
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="sec">Medical / Allergy</div>
          <div style={{ fontSize: 14, marginBottom: 10 }}>Any allergies or medical conditions?</div>
          <div style={{ display: "flex", gap: 8, marginBottom: form.hasAllergy === "yes" ? 12 : 0 }}>
            {["yes", "no"].map(o => <div key={o} className={`ropt${form.hasAllergy === o ? " on" : ""}`} onClick={() => f("hasAllergy", o)}>{o === "yes" ? "Yes" : "No"}</div>)}
          </div>
          {form.hasAllergy === "yes" && <div className="fg"><label>Describe</label><textarea rows={2} value={form.allergyNote} onChange={e => f("allergyNote", e.target.value)} placeholder="e.g. Peanut allergy, EpiPen required..." /></div>}
        </div>

        {/* Notes */}
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="sec">Notes (optional)</div>
          <div className="fg"><label>Internal notes</label><textarea rows={2} value={form.notes} onChange={e => f("notes", e.target.value)} placeholder="e.g. Needs extra support, sibling of..." /></div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {!cameraActive && <button className="btn bo bsm" onClick={startCamera}>Camera</button>}
              {cameraActive && <button className="btn bp bsm" onClick={capturePhoto}>Capture</button>}
              {cameraActive && <button className="btn bg bsm" onClick={stopCamera}>Cancel</button>}
              {form.photo && !cameraActive && <button className="btn bg bsm" onClick={() => f("photo", null)}>Remove</button>}
            </div>
          </div>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div className="fg">
            <label>Teacher *</label>
            <select value={form.teacherId} onChange={e => { const t = eligibleTeachers.find(x => x.id === e.target.value); f("teacherId", e.target.value); f("teacherName", t ? t.name : ""); f("monthlyRate", t ? (t.monthlyRate || 0) : 0); }} disabled={isJunior && form.level === ""}>
              <option value="">{isJunior && form.level === "" ? "Select level first" : "Select teacher..."}</option>
              {eligibleTeachers.map(t => <option key={t.id} value={t.id}>{t.name + (t.monthlyRate ? ` — $${t.monthlyRate}/mo` : "")}</option>)}
            </select>
          </div>
        ...
        {!form.editingEnrollId && (
          <div className="card" style={{ marginBottom: 22 }}>
            ...
            <div className="r2" style={{ marginBottom: 12 }}>
              <div className="fg"><label>Type *</label><select value={form.paymentType} onChange={e => f("paymentType", e.target.value)}><option value="full">Paid in Full</option><option value="partial">Partially Paid</option><option value="instalment">Instalments</option><option value="discounted">Discounted</option><option value="waived">Waived</option></select></div>
              {(form.paymentType === "full" || form.paymentType === "discounted") && <div className="fg"><label>Date</label><input type="date" value={form.paymentDate} onChange={e => f("paymentDate", e.target.value)} /></div>}
            </div>
            ...
            {form.paymentType === "discounted" && <div><div className="r2" style={{ marginBottom: 10 }}><div className="fg"><label>Amount ($)</label><input type="number" value={form.discountedAmount} onChange={e => f("discountedAmount", e.target.value)} /></div><div className="fg"><label>Method</label><select value={form.paymentMethod} onChange={e => f("paymentMethod", e.target.value)}>{PAYMENT_METHODS.map(m => <option key={m}>{m}</option>)}</select></div></div><div className="fg"><label>Note</label><input value={form.paymentNote} onChange={e => f("paymentNote", e.target.value)} placeholder="Optional" /></div></div>}
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn bp" onClick={submitEnrollment}>{form.editingEnrollId ? "Save Changes" : "Enroll Student"}</button>
      ...
      <div className="fade" style={{ padding: pad }}>
        ...
        {families.length === 0 ? <div className="card" style={{ textAlign: "center", padding: 40, color: "#ccc" }}>No families yet.</div>
          : families.map(fam => {
            ...
            const isOpen = selectedFamilyId === fam.id;
            const isCredit = billing.balance < 0;
            return (
                ...
                <div className={`card fr${isOpen ? " open" : ""}`} onClick={() => setSelectedFamilyId(isOpen ? null : fam.id)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#999" }}>{fam.phone && fmtPhone(fam.phone)}{fam.phone && fam.email && " · "}{fam.email}</div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: "#bbb", marginTop: 1 }}>{`${(fam.personIds && fam.personIds.length) || 0} members`}</div>
                    </div>
                    <div style={{ fontSize: 16, color: "#ccc", transition: "transform .2s", transform: isOpen ? "rotate(90deg)" : "none", flexShrink: 0 }}>{">"}</div>
                  </div>
                </div>
                {isOpen && (
                  <div className="fade" style={{ background: "#fff", border: "1px solid #e8f5ee", borderTop: "none", borderRadius: "0 0 12px 12px", padding: "16px", marginTop: -4 }}>
                      ...
                      return (
                        <div key={pid} style={{ border: "1px solid #f0ede6", borderRadius: 10, padding: 12, marginBottom: 10 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}><span className="sn">{person.studentNum}</span><span style={{ fontWeight: 700, fontSize: 13 }}>{`${person.firstName} ${person.lastName}`}</span><span style={{ fontSize: 11, color: "#aaa" }}>{`${person.gender} · Age ${person.age}`}</span></div>
                              ...
                              <div key={e.id} style={{ background: "#fafaf8", borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, flexWrap: "wrap", gap: 4 }}>
                                  ...
                                  <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                                    <span className={eBal > 0 ? "brr" : "bgg"} style={{ fontSize: 11 }}>{`$${eBal.toFixed(2)}`}</span>
                                    <button className="btn bg bxs" onClick={() => openEditEnrollment(e)}>Edit</button>
                                    {["instalment", "partial", "discounted"].includes(e.paymentType) && eBal > 0 && <button className="btn bgold bxs" onClick={() => setPaymentModal({ enrollmentId: e.id })}>+Pay</button>}
                                  </div>
                                </div>
    ...
    if (view === "lookup") return (
      <div className="fade" style={{ padding: pad, maxWidth: 700 }}>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="fg"><label>Name, Phone, or Email</label><input value={lookupQuery} onChange={e => handleLookup(e.target.value)} placeholder="e.g. Ahmed or 6131234567" autoFocus style={{ fontSize: 16 }} /></div>
          {lookupQuery && !lookupResults.length && <div style={{ marginTop: 8, fontSize: 13, color: "#bbb" }}>No results.</div>}
        </div>
          ...
          return (
            <div key={person.id} className="card" style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {fam && <button className="btn bo bsm" onClick={() => { setSelectedFamilyId(fam.id); setView("families"); }}>View Family</button>}
                {pes.map(e => <button key={e.id} className="btn bg bsm" onClick={() => openEditEnrollment(e)}>{`Edit ${PROGRAMS[e.program]}`}</button>)}
              </div>
            </div>
          );
        })}
    ...
    if (view === "teachers") return (
      <div className="fade" style={{ padding: pad }}>
        ...
        <div style={{ display: isMobile ? "block" : "grid", gridTemplateColumns: "1fr 300px", gap: 18 }}>
          <div>
            {["brothers", "sisters"].map(prog => (
              <div key={prog} className="card" style={{ marginBottom: 14, borderTop: `3px solid ${prog === "brothers" ? "var(--bro)" : "var(--sis)"}` }}>
                <div className="sec">{`${PROGRAMS[prog]} — Levels`}</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 10 }}>
                  {(adultLevels[prog] || []).map((l, i) => <span key={i} className="lt">{l}<button onClick={() => setAdultLevels(p => ({ ...p, [prog]: p[prog].filter((_, j) => j !== i) }))}>x</button></span>)}
                  {!(adultLevels[prog] && adultLevels[prog].length) && <span style={{ fontSize: 12, color: "#bbb" }}>No levels yet</span>}
                </div>
                <div style={{ display: "flex", gap: 7 }}>
                  <input value={newLevelInput[prog] || ""} onChange={e => setNewLevelInput(p => ({ ...p, [prog]: e.target.value }))} onKeyDown={e => { if (e.key === "Enter") { const v = (newLevelInput[prog] || "").trim(); if (v) { setAdultLevels(p => ({ ...p, [prog]: [...(p[prog] || []), v] })); setNewLevelInput(p => ({ ...p, [prog]: "" })); } } }} placeholder="New level name..." style={{ flex: 1, padding: "8px 12px", border: "1.5px solid #ddd", borderRadius: 8, fontSize: 14, fontFamily: "inherit" }} />
                  <button className="btn bp bsm" onClick={() => { const v = (newLevelInput[prog] || "").trim(); if (v) { setAdultLevels(p => ({ ...p, [prog]: [...(p[prog] || []), v] })); setNewLevelInput(p => ({ ...p, [prog]: "" })); } }}>+ Add</button>
                </div>
              </div>
            ))}
            {[{ prog: "juniors", tList: juniorTeachers, color: "var(--g)" }, { prog: "brothers", tList: adultTeachers.brothers || [], color: "var(--bro)" }, { prog: "sisters", tList: adultTeachers.sisters || [], color: "var(--sis)" }].map(item => (
              <div key={item.prog} className="card" style={{ marginBottom: 12, borderTop: `3px solid ${item.color}` }}>
                <div className="sec">{`${PROGRAMS[item.prog]} — Teachers`}</div>
                {!item.tList.length ? <div style={{ color: "#ccc", fontSize: 13 }}>No teachers yet.</div>
                  : <div style={{ overflowX: "auto" }}><table className="tbl" style={{ minWidth: 320 }}>
                    <thead><tr><th>Name</th><th>{item.prog === "juniors" ? "Levels" : "Rate"}</th><th>Students</th><th></th></tr></thead>
                    <tbody>{item.tList.map(t => { const cnt = enrollments.filter(e => e.teacherId === t.id && e.active).length; return <tr key={t.id}><td style={{ fontWeight: 600 }}>{t.name}</td><td>{item.prog === "juniors" ? <div style={{ display: "flex", gap: 3 }}>{(t.levels || []).sort((a, b) => a - b).map(li => <span key={li} className="bgg" style={{ fontSize: 10 }}>{JUNIOR_LEVELS[li]}</span>)}</div> : <span className="bgry">{`$${t.monthlyRate}/mo`}</span>}</td><td><span className="bgry">{cnt}</span></td><td><div style={{ display: "flex", gap: 4 }}><button className="btn bg bsm" onClick={() => { setTeacherForm({ program: item.prog, name: t.name, levels: t.levels || [], monthlyRate: t.monthlyRate || "" }); setEditingTeacherId(t.id); }}>Edit</button><button className="btn bd bsm" onClick={() => { if (!window.confirm("Remove?")) return; if (item.prog === "juniors") setJuniorTeachers(p => p.filter(x => x.id !== t.id)); else setAdultTeachers(p => ({ ...p, [item.prog]: p[item.prog].filter(x => x.id !== t.id) })); }}>X</button></div></td></tr>; })}</tbody>
                  </table></div>}
              </div>
            ))}
          </div>
          <div className={`card${isMobile ? "" : ""}`} style={{ position: isMobile ? "static" : "sticky", top: 20, marginTop: isMobile ? 14 : 0 }}>
            ...
            <div style={{ display: "flex", gap: 7 }}>
              <button className="btn bp" onClick={saveTeacher}>{editingTeacherId ? "Save" : "Add Teacher"}</button>
              {editingTeacherId && <button className="btn bg" onClick={() => { setTeacherForm({ program: "juniors", name: "", levels: [], monthlyRate: "" }); setEditingTeacherId(null); setTeacherMsg(""); }}>Cancel</button>}
            </div>
          </div>
        </div>
      </div>
    ...
    if (view === "mailing") {
      const emailMap = {};
      persons.forEach(p => {
        const progEnrolls = enrollments.filter(e => e.personId === p.id && e.active);
      ...
      const allEmails = entries.map(e => e[0]).join(", ");
      return (
        <div className="fade" style={{ padding: pad }}>
          ...
          {entries.length === 0 ? <div className="card" style={{ textAlign: "center", padding: 40, color: "#ccc" }}>No emails yet.</div>
            : <div>
              <div className="card" style={{ marginBottom: 14 }}>
                <div className="sec">All Emails</div>
                <textarea readOnly value={allEmails} rows={3} onClick={e => { e.target.select(); document.execCommand("copy"); }} style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #ddd", borderRadius: 8, fontSize: 12, fontFamily: "monospace", background: "#fafafa", cursor: "pointer", resize: "none", color: "#1a1a1a" }} />
                <div style={{ fontSize: 11, color: "#aaa", marginTop: 4 }}>Click to copy all</div>
                <a href={`mailto:?bcc=${encodeURIComponent(allEmails)}`} style={{ display: "inline-block", marginTop: 8, padding: "8px 16px", background: "var(--g)", color: "#fff", borderRadius: 6, fontSize: 13, fontWeight: 600, textDecoration: "none" }}>Open in Mail App (BCC All)</a>
              </div>
              <div className="card" style={{ overflowX: "auto" }}>
                <div className="sec">All Contacts</div>
                <table className="tbl" style={{ minWidth: 320 }}>
                  <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Role</th></tr></thead>
                  <tbody>{entries.map((entry, i) => { const email = entry[0]; const info = entry[1]; return <tr key={email}><td style={{ color: "#bbb", fontSize: 11 }}>{i + 1}</td><td style={{ fontWeight: 600 }}>{info.name || "-"}</td><td><a href={`mailto:${email}`} style={{ color: "var(--g)", textDecoration: "none", fontSize: 12 }}>{email}</a></td><td><span className="bgry" style={{ fontSize: 10 }}>{info.role}</span></td></tr>; })}</tbody>
                </table>
              </div>
            </div>}
        </div>
    ...
    if (view === "settings") return (
      <div className="fade" style={{ padding: pad, maxWidth: 520 }}>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="sec">Current Semester</div>
          <div className="fg" style={{ marginBottom: 12 }}><label>Semester Name</label><input value={semesterLabel} onChange={e => setSemesterLabel(e.target.value)} placeholder="e.g. Fall 2025 or Winter 2026" /></div>
          <div className="fg"><label>Semester Length (months)</label><input type="number" min={1} max={12} value={semesterMonths} onChange={e => setSemesterMonths(parseInt(e.target.value) || 5)} placeholder="5" /></div>
        </div>
        <div className="card">
          <div className="sec">School Information</div>
          <div style={{ fontSize: 14, marginBottom: 4 }}><strong>{SCHOOL_NAME}</strong></div>
          <div style={{ fontSize: 13, color: "#888" }}>251 Northwestern Ave</div>
          <div style={{ fontSize: 13, color: "#888" }}>Ottawa, ON, K1Y 0M1</div>
          <div style={{ fontSize: 13, color: "#888", marginTop: 3 }}>tarteel.ca</div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ fontFamily: "'Red Hat Display', sans-serif", background: "#f8f6f1", minHeight: "100vh", width: "100%" }}>
      ...
      {/* PAYMENT MODAL */}
      {paymentModal && (() => {
        const enroll = enrollments.find(e => e.id === paymentModal.enrollmentId);
        const person = enroll ? persons.find(p => p.id === enroll.personId) : null;
        const bal = enroll ? enrollBalance(enroll) : 0;
        if (!enroll || !person) return null;
        return (
          <div className="mbg" onClick={() => setPaymentModal(null)}>
            <div className="modal fade" onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
                <h2 style={{ fontSize: 19, fontWeight: 700 }}>Record Payment</h2>
                <button className="btn" onClick={() => setPaymentModal(null)} style={{ fontSize: 20, color: "#bbb", padding: "0 6px" }}>x</button>
              </div>
              <div style={{ background: "#f8f6f1", borderRadius: 10, padding: 12, marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3 }}><span className="sn">{person.studentNum}</span><span style={{ fontWeight: 700 }}>{`${person.firstName} ${person.lastName}`}</span></div>
                <div style={{ fontSize: 12, color: "#888" }}>{`${PROGRAMS[enroll.program]} — ${enroll.teacherName}`}</div>
                <div style={{ fontSize: 13, marginTop: 3 }}>{"Balance: "}<span style={{ color: "var(--red)", fontWeight: 700 }}>{`$${bal.toFixed(2)}`}</span></div>
              </div>
              <div className="r2" style={{ marginBottom: 12 }}>
                <div className="fg"><label>Amount ($) *</label><input type="number" value={instalment.amount} onChange={e => setInstalment(p => ({ ...p, amount: e.target.value }))} placeholder={`Up to $${bal.toFixed(2)}`} autoFocus /></div>
                <div className="fg"><label>Date *</label><input type="date" value={instalment.date} onChange={e => setInstalment(p => ({ ...p, date: e.target.value }))} /></div>
              </div>
              <div className="fg" style={{ marginBottom: 12 }}><label>Method</label><select value={instalment.method} onChange={e => setInstalment(p => ({ ...p, method: e.target.value }))}>{PAYMENT_METHODS.map(m => <option key={m}>{m}</option>)}</select></div>
              <div className="fg" style={{ marginBottom: 12 }}><label>Note</label><input value={instalment.note} onChange={e => setInstalment(p => ({ ...p, note: e.target.value }))} placeholder="Optional" /></div>
              <div style={{ background: "#fef5e4", border: "1px solid #f0d080", borderRadius: 8, padding: 10, marginBottom: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--gold)", marginBottom: 6 }}>Override Balance (optional)</div>
                <div className="fg"><label>Custom balance ($)</label><input type="number" value={customBal} onChange={e => setCustomBal(e.target.value)} placeholder={`Auto: $${Math.max(0, bal - (parseFloat(instalment.amount) || 0)).toFixed(2)}`} /></div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button className="btn bp" onClick={addPayment}>Confirm & Receipt</button>
                <button className="btn bg" onClick={() => { setPaymentModal(null); setCustomBal(""); }}>Cancel</button>
              </div>
            </div>
          </div>
        );
      ...
      {/* EDIT PAYMENT MODAL */}
      {editPaymentModal && (() => {
        const enroll = enrollments.find(e => e.id === editPaymentModal.enrollmentId);
        const person = enroll ? persons.find(p => p.id === enroll.personId) : null;
        if (!enroll || !person) return null;
        return (
          <div className="mbg" onClick={() => setEditPaymentModal(null)}>
            <div className="modal fade" onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
                <h2 style={{ fontSize: 19, fontWeight: 700 }}>Edit Payment</h2>
                <button className="btn" onClick={() => setEditPaymentModal(null)} style={{ fontSize: 20, color: "#bbb", padding: "0 6px" }}>x</button>
              </div>
              <div style={{ background: "#f8f6f1", borderRadius: 10, padding: 10, marginBottom: 14, fontSize: 13 }}><span className="sn" style={{ marginRight: 7 }}>{person.studentNum}</span><strong>{`${person.firstName} ${person.lastName}`}</strong></div>
              <div className="r2" style={{ marginBottom: 12 }}>
                <div className="fg"><label>Amount ($)</label><input type="number" step="0.01" value={editPaymentForm.amount} onChange={e => setEditPaymentForm(p => ({ ...p, amount: e.target.value }))} autoFocus /></div>
                <div className="fg"><label>Date</label><input type="date" value={editPaymentForm.date} onChange={e => setEditPaymentForm(p => ({ ...p, date: e.target.value }))} /></div>
              </div>
              <div className="fg" style={{ marginBottom: 12 }}><label>Method</label><select value={editPaymentForm.method} onChange={e => setEditPaymentForm(p => ({ ...p, method: e.target.value }))}>{PAYMENT_METHODS.map(m => <option key={m}>{m}</option>)}</select></div>
              <div className="fg" style={{ marginBottom: 16 }}><label>Note</label><input value={editPaymentForm.note} onChange={e => setEditPaymentForm(p => ({ ...p, note: e.target.value }))} placeholder="Optional" /></div>
              <div style={{ display: "flex", gap: 8 }}>
                <button className="btn bp" onClick={saveEditedPayment}>Save Changes</button>
                <button className="btn bg" onClick={() => setEditPaymentModal(null)}>Cancel</button>
              </div>
            </div>
          </div>
        );
      ...
      {receiptModal && (
        <ReceiptView person={receiptModal.person} enrollment={receiptModal.enrollment} payment={receiptModal.payment} receiptNum={receiptModal.receiptNum} semesterLabel={semesterLabel} onClose={() => setReceiptModal(null)} />
      )}
    </div>
  );
}
